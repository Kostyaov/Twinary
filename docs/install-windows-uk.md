...
2. Вкажи назву профілю. У полі є підказка `Name Project`.
3. Для `Computer folder` вибери папку на комп'ютері кнопкою з іконкою папки.
4. Для `External folder` вибери папку на зовнішньому диску кнопкою з іконкою папки.
5. Натисни `Save`.
6. Натисни `Analyze`.
7. Перевір `Sync Plan`.
...
`Analyze` тільки готує план і нічого не копіює. `Synchronize` виконує підготовлений план.

Якщо аналіз або синхронізація триває довго, можна натиснути `Stop`. Зупинка може спрацювати з невеликою затримкою, бо backend завершує поточний файловий крок.

## 9. Що Означає Strict Verification

`Strict verification` - це обережніший режим перевірки однакових файлів.
...
Якщо `Strict verification` увімкнений, BackupFlow може додатково порахувати хеш вмісту для малих не-медійних файлів, коли розмір однаковий, а дата редагування відрізняється.
