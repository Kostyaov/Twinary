...
Службові логи launcher-а зберігаються локально в папці:
...
- запустить Python backend у фоні
...
2. Вкажи назву профілю. У полі є підказка `Назва проєкту`.
3. Для `Папка на комп'ютері` вибери папку на комп'ютері кнопкою з іконкою папки.
4. Для `Папка на зовнішньому диску` вибери папку на зовнішньому диску кнопкою з іконкою папки.
5. Натисни `Зберегти`.
6. Натисни `Аналіз`.
7. Перевір `План синхронізації`.
8. Натисни `Синхронізувати`.
9. Після завершення знову натисни `Аналіз`.
...
`Аналіз` тільки готує план і нічого не копіює. `Синхронізувати` виконує підготовлений план.

Якщо аналіз або синхронізація триває довго, можна натиснути `Стоп`. Зупинка може спрацювати з невеликою затримкою, бо backend завершує поточний файловий крок.

## 9. Що Означає Strict Verification

В українському інтерфейсі ця опція називається `Сувора перевірка`. В англійському - `Strict verification`.

`Сувора перевірка` - це обережніший режим перевірки однакових файлів.

Звичайний режим порівнює:

- шлях файлу;
- розмір;
- дату останнього редагування.

Якщо `Сувора перевірка` увімкнена, BackupFlow може додатково порахувати хеш вмісту для малих не-медійних файлів, коли розмір однаковий, а дата редагування відрізняється.

Це допомагає уникати зайвих повторних копіювань, але може бути повільнішим.

Великі файли, відео, аудіо, архіви й образи дисків перевіряються швидким способом навіть у strict-режимі, щоб не читати великі файли повністю.
...
Логи прихованого backend-процесу під час запуску через `start-windows.bat`:
...
1. Закрий BackupFlow.
2. Відкрий `Command Prompt` або `PowerShell`.
...
Якщо ти встановив проєкт в іншу папку, заміни шлях у команді `cd` на свій.
...
### Відкривається тільки terminal, а головне вікно ні

Подивись останні рядки у terminal. Найчастіші причини:

- не встановлений Rust;
- не встановлений Node.js;
- залежності не встигли встановитися;
- Windows блокує порт backend-а.
...
### Після `git pull` щось не запускається

Спробуй закрити всі вікна BackupFlow і запустити `start-windows.bat` ще раз.

Якщо проблема схожа на залежності frontend-а, можна видалити:

```text
Twinary\apps\desktop\node_modules
```

Після цього `start-windows.bat` встановить Node.js залежності заново.

Якщо проблема схожа на Python-залежності, можна видалити:

```text
Twinary\.venv
```

Після цього `start-windows.bat` створить Python-середовище заново.

## 13. Що не варто робити

- Не запускай `pip install` глобально для BackupFlow.
- Не видаляй `.venv`, якщо не хочеш створювати Python-середовище заново.
