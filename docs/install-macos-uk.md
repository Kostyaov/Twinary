...
3. Обери `Computer folder` кнопкою з іконкою папки.
4. Обери `External folder` кнопкою з іконкою папки.
5. За потреби увімкни `Strict verification`.
6. Натисни `Save`.
7. Натисни `Analyze`.
8. Перевір `Sync Plan`.
9. Натисни `Synchronize`.
10. Після завершення знову натисни `Analyze`.
...
`Strict verification` додатково може рахувати хеш вмісту для малих не-медійних файлів, якщо розмір однаковий, а дата редагування різна.
...
- Не створюй профіль, де `Computer folder` і `External folder` вказують на одну й ту саму папку.
